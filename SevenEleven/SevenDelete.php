...
//            查找的动作
            function deleteA(){
                ...
                var deletes = {deleteSname: delete_Sname};
                var myUrl = "SevenDB/SevenDBDeleteInit.php";
                $.ajax({
                    url: myUrl,
                    type: 'POST',
                    data: deletes,
            ...
            function showDelete(inData){
                ...
                str += '<tr><td>店号</td><td>店名</td><td>地址</td><td>电话</td><td>删除</td></tr>';
                    for(var i = 0; i < inData.length; i++){
                        str += '<tr><td>' + inData[i].Sno + '</td>';
                        str += '<td>' + inData[i].Sname + '</td>';
                        str += '<td>' + inData[i].Slocation + '</td>';
                        str += '<td>' + inData[i].Sphone + '</td>';
                        str += "<td><input type='button' value='確定刪除' class='shanchuCSS' onclick='execDelete(" + inData[i]['Sno'] + ",this)' /></td></tr>";
                    }
                    str += '</table>';
                    document.getElementById("demoDelete").innerHTML = str;
            }
            
            function execDelete(sid,haha){
                var deletes = {Sno: sid};
                var myUrl = "SevenDB/SevenDBDelete.php";
                $.ajax({
                    url: myUrl,
                    type: 'POST',
                    data: deletes,
            ...
            <div id="divDelete" class="store">
                <div class="leftInput" style="font-size: 20px;width:40%;">
                    ...
                    <table>
                        <tr>
                            <td>门市店名</td>
                            <td><input type="text" id="delete_Sname" placeholder="请输入门市店名" maxlength="6" class="input"></td>
                        </tr>
                    </table>
                    <table>
                        <tr>
